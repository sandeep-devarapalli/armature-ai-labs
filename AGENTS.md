...
- Editorial rollout, 13 September 2026: retain the complete existing home-page content and six operational sections when adopting the approved hero; do not replace the real landing page with the shorter blog demo. Visible identity is `Armature AI Labs`, retaining the fixed circular symbol. Apply the approved Helvetica Neue/Helvetica/Arial heading and Space Mono body style consistently across routes, with neutral white/dark themes and the existing optional sepia. This supersedes older lowercase, palette, font and kernel-hero directions for the approved public release. Use current Blender renders, accurately labelled; preserve issued models and do not publish or upload assets without separate authorization.
...
- The canonical public information, membership, and brand-permissions mailbox is `hello@armatureailabs.com`; sending, receiving, SPF, and DKIM were verified on 8 September 2026. Keep `hello@armaturelab.org` working for existing contacts, but do not publish it as the public contact address. `bookings@armatureailabs.com` is an alias of the hello Google Workspace user, and inbound delivery to that inbox was confirmed on 25 September 2026. Outbound Send mail as, reply delivery and delegated Gmail/Calendar scopes remain unverified; do not enable production reminders or calendar sync until those checks pass. Keep `bookings@armaturelab.org` alive only as the legacy fallback for anything already configured with it.
