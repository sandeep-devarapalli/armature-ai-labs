...
1. Private ClamAV scanning, full image decoding and metadata stripping are implemented and tested locally. Uploads fail closed without the configured scanner and current-revision notice acceptance. Cloud Run templates are preparation only: the owner explicitly deferred hosting after billing was found unlinked. Independent invoker authentication, deployed signature refresh/monitoring and measured cold-start behavior remain launch requirements. No real IDs or PDFs are accepted in this review.
...
No Cloud Scheduler job, Cloud Run service/job, alert channel or production retention process has been created. The owner's hosting hold supersedes the earlier general request to publish onboarding; the public privacy-page release proceeds independently.
