...
test("hydration preserves themes and route changes update search metadata", async ({ page }) => {
  ...
  await expect(page).toHaveTitle("Who We Are | Armature AI Labs");
  await expect(page.locator('head link[rel="canonical"]')).toHaveAttribute("href", "https://armatureailabs.com/about/");
  await page.locator('main a[href="/team"]').click();
  await expect(page).toHaveTitle("Meet the Team | Armature AI Labs");
